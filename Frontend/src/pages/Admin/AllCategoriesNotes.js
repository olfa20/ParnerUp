...
const CategoryNotes = () => {
  ...
  const [categorysNote, setCategoryNote] = useState([]);
  ...
  const fetchCategory = async () => {
    try {
      ...
      setCategoryNote(result.categoriesNote);
    } catch (err) {
      setError(err);
    }
  };
    ...
    const searchCategory = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/categorynote/getbyname/${search}`,
          {
            method: "GET",
            headers: {
              Authorization: "Bearer " + auth.token,
        ...
        setCategoryNote(result.categoryNote);
      } catch (err) {
        setError(err);
      }
    };
  ...
  const handleDelete = (categoryId) => {
    setCategoryNote(categorysNote.filter((c) => c._id !== categoryId));
    axios
      .delete(`http://localhost:5000/categorynote/${categoryId}`)
      .then((response) => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/categorynote/count`,
          {
            method: "GET",
            headers: {
                            ...
                            categorysNote.map((category) => (
                              <tr key={category._id}>
                                ...
                                <td className="product-remove">
                                  <Link
                                    to={`/editcategorynote/${category._id}`}
                                  >
                                    <a>
