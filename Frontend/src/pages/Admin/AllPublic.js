...
const Public = () => {
    ...
    const searchPublic = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/public/getpublicbyname/${search}`,
          {
            method: "GET",
            headers: {
              Authorization: "Bearer " + auth.token,
  ...
  const handleDelete = (publicId) => {
    setPublics(publics.filter((c) => c._id !== publicId));
    axios
      .delete(`http://localhost:5000/public/${publicId}`)
      .then((response) => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const response = await fetch(`http://localhost:5000/public/count`, {
          method: "GET",
          headers: {
                            ...
                            publics.map((publice) => (
                              <tr key={publice._id}>
                                ...
                                <td className="product-remove">
                                  <Link to={`/editpublic/${publice._id}`}>
                                    <a>
                                ...
                                <td className="product-remove">
                                  <a onClick={() => handleDelete(publice._id)}>
