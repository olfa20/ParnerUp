...
const AllAdvanced = () => {
  ...
  const fetchAdvanced = async () => {
    try {
      const response = await fetch(`http://localhost:5000/advanced/all`, {
        method: "GET",
        headers: {
  ...
  const handleDelete = (advancedId) => {
    setAdvanceds(advanceds.filter((s) => s._id !== advancedId));
    axios
      .delete(`http://localhost:5000/advanced/${advancedId}`)
      .then((response) => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const response = await fetch(`http://localhost:5000/advanced/count`, {
          method: "GET",
          headers: {
    ...
    const searchAdvanced = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/advanced/title/${search}`,
          {
            method: "GET",
            headers: {
              Authorization: "Bearer " + auth.token,
                      ...
                      <table className="table text-center">
                        <thead className="bg-greyblue rounded-3">
                          <tr>
                            ...
                            <th className="border-0 p-4">userType</th>
                            ...
                            advanceds.map((advanced) => (
                              <tr key={advanced._id}>
                                ...
                                <td className="product-remove">
                                  <Link to={`/editadvanced/${advanced._id}`}>
                                    <a>
                                ...
                                <td className="product-remove">
                                  <a onClick={() => handleDelete(advanced._id)}>
